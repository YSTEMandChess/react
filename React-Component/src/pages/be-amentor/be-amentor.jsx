...
const BoardAnalyzer = () => {
    ...
    useEffect(() => {
        const boardConfig = {
            ...
            onDrop: onDrop,
            onSnapEnd: onSnapEnd,
            //pieceTheme: `${process.env.PUBLIC_URL}/assets/images/chessPieces/wikipedia/{piece}.png`,
        };
    ...
    const onDrop = (source, target) => {
        ...
        const moveAudioSrc = `${process.env.PUBLIC_URL}/assets/audio/move.ogg`;
        const captureAudioSrc = `${process.env.PUBLIC_URL}/assets/audio/capture.ogg`;
        const slideAudioSrc = `${process.env.PUBLIC_URL}/assets/audio/slider.ogg`;
                    ...
                    <div id="move-list">
                        {plies.map((ply, i) => (
                            <div className="ply" key={i}>
                                <span>{i + 1}.</span>
                                {ply.map((move, j) => (
                                    <span key={j} onClick={() => setCurrMove(move)} className={`move ${move === currMove ? 'selected' : ''}`} id={`move-${i}-${j}`}>
                                        {move.move}
                                    </span>
                                ))}
                            </div>
