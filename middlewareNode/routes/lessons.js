    ...
    try {
      ...
      if (req.user) {
        ...
        const userDoc = await users.findOne(
          // get the userDoc according to username
          { username: username },
          { projection: { lessonsCompleted: 1 } }
        );
        if (!userDoc) throw new Error("User does not exist");
        if (!userDoc.lessonsCompleted)
          throw new Error("User does not have lesson record");
        ...
        await guests.updateOne(
          { ip: clientIp },
          {
            ...
            $setOnInsert: {
              // if this ip is not in db, add lessonsCompelted field
              lessonsCompleted: [
                {
                ...
                {
                  piece: "Double Check A very powerfull tactic",
                  lessonNumber: 0,
                },
                {
                ...
                {
                  piece: "X-Ray Attacking through an enemy piece",
                  lessonNumber: 0,
                },
                ...
                {
                  piece: "Attraction Lure a piece to bad square",
                  lessonNumber: 0,
                },
                {
                ...
                {
                  piece: "Undermining Remove the defending piece",
                  lessonNumber: 0,
                },
                ...
                {
                  piece: "Basic Rook Endgames Lucena and Philidor",
                  lessonNumber: 0,
                },
              ],
            },
        ...
        if (lessonNum == -1)
          return res.status(400).json("Error: 400. Invalid piece.");
    ...
    try {
      ...
      if (req.user) {
        ...
        userDoc.lessonsCompleted.forEach((lesson, i) => {
          // try finding user's progress for that piece
        ...
        const updateResult = await users.updateOne(
          {
            ...
            $or: [
              {
                [`lessonsCompleted.${index}.lessonNumber`]: {
                  $lt: lessonNum + 1,
                },
              },
              {
        ...
        await guests.updateOne(
          { ip: clientIp },
          {
            ...
            $setOnInsert: {
              // add lessonsCompleted field if ip is not stored before
              lessonsCompleted: [
                {
                  piece: "Piece Checkmate 1 Basic checkmates",
                  lessonNumber: 0,
        ...
        let index = -1; // index for that piece
        guestDoc.lessonsCompleted.forEach((lesson, i) => {
          // try finding user's progress for that piece
          if (lesson.piece === piece) {
            index = i;
          }
    ...
    try {
      ...
      if (req.user) {
        ...
        const lessonsMap = Object.fromEntries(
          userDoc.lessonsCompleted.map(({ piece, lessonNumber }) => [
            piece,
            lessonNumber,
          ])
        );
        ...
        if (!guestDoc) throw new Error("Guest does not exist");
        if (!guestDoc.lessonsCompleted)
          throw new Error("Guest does not have lesson record");

        // create the map
        const lessonsMap = Object.fromEntries(
          guestDoc.lessonsCompleted.map(({ piece, lessonNumber }) => [
            piece,
            lessonNumber,
          ])
        );
