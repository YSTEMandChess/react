...
const ScenarioTemplate = React.memo(function ScenarioTemplate({
  scenario,
  onClick,
  styles,
}) {
...
const LessonTemplate = React.memo(function LessonTemplate({
  lesson,
  onClick,
  styles,
}) {
...
export default function LessonSelection({ onGo, styleType = "page" }) {
  // what to do when clicking go button, default to navigation
    ...
    async function fetchLessonsForScenario() {
      ...
      if (currentScenario) {
        ...
        if (unlocked === 0 || unlocked == null) {
          ...
          for (
            let j = 0;
            j <= unlocked && j < currentScenario.subSections.length;
            j++
          ) {
            if (currentScenario.subSections[j]) {
  ...
  return (
    <div className={styles.wholePage}>
      ...
      >
        ...
        <div style={{ marginRight: "1rem" }}>{showScenarios ? "▼" : "▲"}</div>
      </div>
      ...
      >
        ...
        <div style={{ marginRight: "1rem" }}>{showLessons ? "▼" : "▲"}</div>
      </div>
