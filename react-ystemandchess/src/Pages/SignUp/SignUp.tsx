...
const StudentTemplate: React.FC<StudentTemplateProps> = ({
  studentUsername,
  onClick,
}) => {
  return (
...
const Signup = () => {
  ...
  const [assignedMenteeUsername, setAssignedMenteeUsername] = useState<
    string | null
  >(null); // To store the selected mentee's username
  ...
  const handleRemoveStudent = (studentId: any) => {
    ...
    if (students.length === 1) {
      // If only one student left, and it's removed, hide the form
  ...
  const handleSubmit = async () => {
    ...
    if (!isValid) {
      ...
      setErrors((prev) => ({
        ...prev,
        general: "Please correct the form errors.",
      }));
      return;
    }
    ...
    try {
      ...
      if (!signupResponse.ok) {
        ...
        if (
          ...
          throw new Error(
            `HTTP error! status: ${signupResponse.status}, message: ${errorMessage}`
          );
        }
        return;
      }
      ...
      try {
        ...
        if (!loginResponse.ok) {
          ...
          throw new Error(
            `Login failed after signup: ${
              loginErrorData.message || "Unknown login error"
            }`
          );
        }
        ...
        setErrors((prev) => ({
          ...prev,
          general: `Signup successful, but failed to log in to assign mentee: ${
            loginError.message || "Login network error"
          }`,
        }));
      ...
      if (
        ...
        const updateMentorshipUrl = `${
          environment.urls.middlewareURL
        }/user/updateMentorship?mentorship=${encodeURIComponent(
          assignedMenteeUsername
        )}`;
        ...
        try {
          ...
          console.log(
            "Update Mentorship Response status:",
            updateMentorshipResponse.status
          );
          ...
          if (!updateMentorshipResponse.ok) {
            ...
            setErrors((prev) => ({
              ...prev,
              general: `Signup successful, but mentee assignment failed: ${
                updateErrorData.message || "Unknown error"
              }`,
            }));
          } else {
          ...
          setErrors((prev) => ({
            ...prev,
            general: `Signup successful, but mentee assignment network error: ${
  ...
  return (
    <form className="signupForm" onSubmit={(e) => e.preventDefault()}>
      <h2 className="sign-up-title" data-testid="title">
        Sign up
      </h2>

      <div
        className={`errorMessages ${
          Object.values(errors).some(Boolean) ? "visible" : ""
        }`}
      >
      ...
      <div className="account-type" data-testid="account-type">
        ...
        >
          {" "}
          {/* Corrected to handleAccountTypeChange */}
          <option value="mentor" data-testid="account-mentor">
            Mentor
          </option>
          <option value="parent" data-testid="account-parent">
            Parent
          </option>
        </select>
      </div>
      ...
      {!parentAccountFlag && (
        <>
          ...
          />
          {activeDropdown && (
            <div className="dropdown-users" id="mentee-dropdown-container">
              {" "}
              {/* Keep ID for click outside handler */}
              {dropdownLoading ? (
