...
beforeEach(() => {
  global.fetch = jest.fn((url) => {
    ...
    if (url.includes('timeTracking')) {
      return Promise.resolve({
        json: () => Promise.resolve({ eventId: 'mock-event-id' }),
      });
    }
...
test('next & previous lesson', async () => {
  ...
  // go back one lesson
  const backBtn = screen.getByText("Back", { selector: 'p' }).closest('button');
  ...
  const nextBtn = screen.getByText("Next", { selector: 'p' }).closest('button');
  ...
  const nextBtnAgain = screen.getByText("Next", { selector: 'p' }).closest('button');
