...
const LessonOverlay = () => {
    ...
    useEffect(() => {
        ...
        const handleMessage = async (e) => {
            let clientOrigin = "";
            try {
                if (environment.urls.chessClientURL) {
                    clientOrigin = new URL(environment.urls.chessClientURL).origin;
                }
            } catch (err) {
                console.error("Invalid chessClientURL environment config:", err);
            }
            if (e.origin === clientOrigin || e.origin === window.location.origin) {
