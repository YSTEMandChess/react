...
const LessonOverlay: React.FC<LessonOverlayProps> = ({
  ...
  useEffect(() => {
    ...
    const handleMessage = async (e) => {
      if (e.origin === environment.urls.chessClientURL) {
        ...
        if (!lessonStarted) {
        ...
        } else if (typeof e.data === "string" && e.data.startsWith("won")) {
          // for all lesson types, should check if there's checkmate
          if (e.data.split(":")[1] == turnRef.current)
            setShowVPopup(true); // checkmated, complete lesson
          else setShowXPopup(true); // opponent checkmated, restart lesson
        } else if (typeof e.data === "string" && e.data.startsWith("draw")) {
          // for all lesson types, should check if there's a draw
          if (
          ...
          if (lessonTypeRef.current == "promote") {
            // if the goal of lesson is just to promote pawn
            setShowVPopup(true); // lesson completed
          } else {
            // for other lesson types, allow user to choose what the pawn should promote to
            setIsPromoting(true);
            setPromotionSource(e.data.from);
            setPromotionTarget(e.data.to);
          }
        } else if (typeof e.data === "string" && looksLikeFEN(e.data)) {
          // client sends board fen after user makes a move
          // update fens
          ...
          httpGetAsync(
            // get the next opponent move from stockfish
            ...
            (response) => {
              ...
              if (isReady) {
                // sends opponent moved board to client to update UI
  ...
  getCurrentLessonsRef.current = async (lessonNumber) => {
    try {
      ...
      let timeoutId = setTimeout(() => {
        // set time out to prevent fetching from taking too long
        setShowError(true);
      }, 10000);
      ...
      const response = await fetch(
        ...
        }/lessons/getLesson?piece=${piece}&lessonNum=${lessonNumber + 1}`,
        {
          method: "GET",
          headers: { Authorization: `Bearer ${cookies.login}` },
        }
      ...
      if (
        lessonData.info.includes("Checkmate the opponent") ||
        lessonData.name.includes("= Win")
      ) {
        ...
        lessonData.info.includes("Hold the draw") ||
        lessonData.name.includes("Draw")
      ) {
        lessonTypeRef.current = "draw";
      } else {
      ...
      if (!lessonData || lessonData.lessonNum === undefined) {
        alert(
          "Congratulations! You have completed all lessons for this piece."
        );
        return;
      }
  ...
  updateCompletionRef.current = async () => {
    try {
      if (lessonNum === completedNum) {
        // allow back end update only for the first unfinished lesson
  ...
  const nextLesson = () => {
    if (lessonNum < completedNum && lessonNum < totalLessons - 1) {
      // no navigation beyond first uncompleted lesson or last lesson
        ...
        <div className={styles.rightContainer}>
          ...
          <div className={styles.prevNextContainer}>
            {lessonNum <= 0 ? (
              <button
                className={[
                  styles.prevNextLessonButtonInactive,
                  styles.prev,
                ].join(" ")}
              >
                <BackIconInactive />
      ...
      {showVPopup && (
        <div className={styles.popup}>
          <div className={styles.popupContent}>
            ...
            <button className={styles.popupButton} onClick={handleVPopup}>
              OK
            </button>
          </div>
        </div>
      ...
      {showXPopup && !showVPopup && (
        <div className={styles.popup}>
          <div className={styles.popupContent}>
            ...
            <button className={styles.popupButton} onClick={handleXPopup}>
              OK
            </button>
          </div>
        </div>
