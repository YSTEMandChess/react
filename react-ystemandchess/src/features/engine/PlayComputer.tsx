...
const PlayComputer: React.FC = () => {
    ...
    const startGame = () => {
        chessSocket.connected ? setShowSettings(false) : setShowSettings(true);
    };
    ...
    const onOpponentMove = (data: { fen: string; move?: Move }) => {
        if (!yourTurn) {
            ...
            chessSocket.saveGame(gameMetaData.current);
        } else {
    ...
    const handleMove = useCallback((move: Move) => {
        if (yourTurn) {
            try {
                ...
                gameMetaData.current.updatedAt = Date.now().toString();
                chessSocket.saveGame(gameMetaData.current);
                chessSocket.playMove(gameMetaData.current);
                setYourTurn((prev) => !prev);
            } catch (error) {
    ...
    const loadGame = () => {
        if (location.state) {
            ...
            const newGame = chessSocket.getMostRecentGameInfo(savedMeta);
            ...
            chessSocket.saveNewGame(newGame);
