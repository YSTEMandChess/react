    ...
    subSections: [
      {
        ...
        info: 'Keep your pieces on the opposite color squares from the enemy bishop to stay safe. Use your queen to encroach on the king and look for double attacks.',
        solution: null,
        goal: { type: 'CHECKMATE' },
        opponentConstraints: null,
      },
      {
        ...
        info: 'Force the enemy king to the edge of the board while avoiding tricky knight forks.',
        solution: null,
        goal: { type: 'CHECKMATE' },
        opponentConstraints: null,
      },
      ...
      },
      {
        ...
        info: 'When trying to checkmate with two bishops, there are two important principles to follow. One, the bishops are best when they are near the center of the board and on adjacent diagonals. This cuts off the opposing king. Two, the king must be used aggressively, in conjunction with the bishops.',
        solution: null,
        goal: { type: 'CHECKMATE' },
        opponentConstraints: null,
      },
      ...
      },
      {
        ...
        info: 'Two knights can\'t force checkmate by themselves, but if the enemy has a pawn, we can avoid stalemate and force mate.',
        solution: null,
        goal: { type: 'CHECKMATE' },
        opponentConstraints: null,
      }
  ...
  },
  {
    ...
    subSections: [
      ...
      },
      {
        ...
        solution: 'Rxh3+ Kg1 Rg3 Qxg3 hxg3',
        goal: null,
      },
    ]
  },
...
export function getScenario(index) {
  const scenario = scenariosArray[index]
  const sublen = scenario.subSections.length
  scenario.subSections = scenario.subSections.map(sub => ({
    ...sub,
    board: parseFEN(sub.fen),
    left_ended: index === 0,
    right_ended: index === sublen - 1
  }))
  return scenario
}

export function getScenarioByName(name) {
  return scenariosArray.find(scenario => scenario.name === name);
}

export function getAllScenarios() {
  return scenariosArray.map((_, i) => getScenario(i)).filter(Boolean)
}
